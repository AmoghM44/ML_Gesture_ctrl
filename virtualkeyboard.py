...
keyboard = Controller()
lastKeyPressTime = time.time()  # Variable to store the time of the last key press
while True:
    if counter >0:
        counter -=1
        ...
        for k in keys:
            if k.isOver(mouseX, mouseY) or k.isOver(signTipX, signTipY):
                ...
                if (k.isOver(thumbTipX, thumbTipY)):
                    clickTime = time.time()
                    if clickTime - previousClick > 0.4:  # Adding a delay of 0.4 seconds before allowing the next click                              
                        if k.text == '<--':
                            textBox.text = textBox.text[:-1]
                        elif k.text == 'clr':
                            textBox.text = ''
